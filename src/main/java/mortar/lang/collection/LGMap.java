...
public class LGMap<K, V> extends LinkedHashMap<K, V>
{
	 ...
	 */
	public LGMap<K, V> copy()
	{
		LGMap<K, V> m = new LGMap<K, V>();
	 ...
	 */
	public LGMap<V, GList<K>> flip()
	{
		LGMap<V, GList<K>> flipped = new LGMap<V, GList<K>>();
	 ...
	 */
	public LGMap<K, V> append(LGMap<K, V> umap)
	{
		for(K i : umap.keySet())
		{
			put(i, umap.get(i));
	 ...
	 * @return the modified map
	 */
	public LGMap<K, V> removeDuplicateValues()
	{
		LGMap<K, V> map = this.copy();
