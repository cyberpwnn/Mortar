...
public class EntityNMS12 {
	...
	public static void registerEntity(Type type, Class<? extends Entity> customClass, boolean override) {
		EntityNMS12.registerEntity(type.getName(), type, customClass, override);
	}
	...
	public static void registerEntity(String name, Type type, Class<? extends Entity> customClass, boolean override) {
		...
		try {
			field.setAccessible(true);
			for (BiomeBase base : EntityNMS12.BIOMES) {
				List<BiomeMeta> list = (List<BiomeMeta>) field.get(base);
				for (BiomeMeta meta : list) {
					if (meta.b == type.getNMSClass()) {
						meta.b = (Class<? extends EntityInsentient>) customClass;
	...
	public enum MobMeta {
		MONSTER(EntityNMS12.META_LIST_MONSTER),
		CREATURE(EntityNMS12.META_LIST_CREATURE),
		WATER_CREATURE(EntityNMS12.META_LIST_WATER_CREATURE),
		AMBIENT(EntityNMS12.META_LIST_AMBIENT),
		UNDEFINED(null);
	...
	static {
		...
		MINECRAFT_SERVER = EntityNMS12.CRAFTBUKKIT_SERVER.getServer();
		BIOMES = new BiomeBase[BiomeBase.i.a()];
		Iterator<BiomeBase> iterator = BiomeBase.i.iterator();
		int index = 0;
		while (iterator.hasNext()) {
			EntityNMS12.BIOMES[index++] = iterator.next();
		}
	}
}